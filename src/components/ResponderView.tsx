...
type Props = RN.ViewProps & {
  ...
  enabled: boolean;
  thumbSize?: number;
  onMove: (value: number, containerSize: number) => void;
  onPress: (value: number, containerSize: number) => void;
  onRelease: (value: number, containerSize: number) => void;
...
const ResponderView = React.forwardRef<RN.View, Props>((props, ref) => {
  ...
  const onMove = useEvent((event: RN.GestureResponderEvent) => {
    const size = containerSize.current?.[isVertical ? 'height' : 'width'];
    // Only pass valid size, otherwise pass a large default to effectively disable grabRadius
    onMoveProp(eventToValue(event), size || 10000);
  ...
  const onPress = useEvent((event: RN.GestureResponderEvent) => {
    const size = containerSize.current?.[isVertical ? 'height' : 'width'];
    // Only pass valid size, otherwise pass a large default to effectively disable grabRadius
    onPressProp(eventToValue(event), size || 10000);
    event.preventDefault();
    event.stopPropagation();
  });
  ...
  const onRelease = useEvent((event: RN.GestureResponderEvent) => {
    const size = containerSize.current?.[isVertical ? 'height' : 'width'];
    // Only pass valid size, otherwise pass a large default to effectively disable grabRadius
    onReleaseProp(eventToValue(event), size || 10000);
    event.preventDefault();
    event.stopPropagation();
  });
